...
function edadYTerminos() {
  ...
  /* La invitación a crear cuenta sube sola al llegar al final de la home:
     primero se enseña lo que hay y el final del scroll no queda en seco.
     Una sola vez y solo a quien no tiene cuenta. El disparador invisible
     de la home sigue funcionando igual. */
  const scrollHome = document.querySelector("#home .screen__scroll");
  let invitacionMostrada = false;
  if (scrollHome) {
    scrollHome.addEventListener("scroll", () => {
      if (invitacionMostrada || PT.state.tieneCuenta) return;
      /* Solo cuenta si la home está a la vista: al salir de ella el scroll
         se reinicia y no debe leerse como "ha llegado al final". */
      if (PT.pantallaActual() !== "home" || !scrollHome.clientHeight) return;
      const alFinal = scrollHome.scrollTop + scrollHome.clientHeight >= scrollHome.scrollHeight - 8;
      if (!alFinal) return;
      invitacionMostrada = true;
      PT.abrirHoja("modal-cuenta");
    });
  }
}
...
function miniPlayer() {
  ...
  function mostrar(id) {
    ...
    barra.querySelector(".mini-player__abrir").dataset.contenido = id; // abre esta pieza, no la última vista
