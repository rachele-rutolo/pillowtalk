...
- [PRD – Pillowtalk](#prd--pillowtalk)
  - [1. Resumen Ejecutivo](#1-resumen-ejecutivo)
  - [2. Contexto](#2-contexto)
    - [2.1 Contexto corporativo / de producto](#21-contexto-corporativo--de-producto)
    - [2.2 Nuestro producto](#22-nuestro-producto)
  - [3. People Problem](#3-people-problem)
    - [3.1 Problem Statement](#31-problem-statement)
    - [3.2 Evidencia](#32-evidencia)
    - [3.3 Citas directas](#33-citas-directas)
    - [3.4 Quién tiene este problema](#34-quién-tiene-este-problema)
    - [3.5 Cómo lo resuelve hoy](#35-cómo-lo-resuelve-hoy)
    - [3.6 Oportunidad](#36-oportunidad)
  - [4. Objetivos de Negocio y Métricas de Éxito](#4-objetivos-de-negocio-y-métricas-de-éxito)
    - [4.1 Objetivos del proyecto](#41-objetivos-del-proyecto)
    - [4.2 Qué cambia para el usuario](#42-qué-cambia-para-el-usuario)
    - [4.3 Success Metrics (KPI)](#43-success-metrics-kpi)
    - [4.4 Out of Scope (V4)](#44-out-of-scope-v4)
  - [5. Usuarios e Investigación](#5-usuarios-e-investigación)
    - [5.1 Target audience / Persona](#51-target-audience--persona)
    - [5.2 Insights de investigación](#52-insights-de-investigación)
    - [5.3 User needs / Jobs to be done](#53-user-needs--jobs-to-be-done)
  - [6. Solución Propuesta](#6-solución-propuesta)
    - [6.1 Visión general de la solución](#61-visión-general-de-la-solución)
      - [Arquitectura de las secciones](#arquitectura-de-las-secciones)
      - [Home: decisiones y justificación](#home-decisiones-y-justificación)
      - [Mini player](#mini-player)
      - [Tres formatos, tres tratamientos](#tres-formatos-tres-tratamientos)
    - [6.2 User Flows principales](#62-user-flows-principales)
      - [El recorrido, pantalla a pantalla](#el-recorrido-pantalla-a-pantalla)
      - [Reglas que el diseño no puede romper](#reglas-que-el-diseño-no-puede-romper)
    - [6.3 User Stories (índice priorizado)](#63-user-stories-índice-priorizado)
    - [6.4 Requisitos No Funcionales](#64-requisitos-no-funcionales)
    - [6.5 Edge cases](#65-edge-cases)
  - [7. Aspectos Técnicos](#7-aspectos-técnicos)
    - [7.1 Arquitectura / Enfoque técnico](#71-arquitectura--enfoque-técnico)
    - [7.2 Dependencias](#72-dependencias)
    - [7.3 Riesgos técnicos](#73-riesgos-técnicos)
  - [8. Riesgos y Asunciones](#8-riesgos-y-asunciones)
    - [8.1 Asunciones](#81-asunciones)
    - [8.2 Riesgos](#82-riesgos)
...
La demo se presenta como la sesión de Lola. Todo el contenido visible tiene que ser coherente con ella (ver 7.3).
...
| Temas | Deseo y fantasías · Límites · Comunicación en pareja |
| Qué la ha traído aquí | Falta de deseo · Mejorar la comunicación con mi pareja |
| Permisos y notificaciones | Acepta todo: la demo enseña el caso de uso completo |

El perfil no es hetero a propósito: así la misma demo sirve de prueba del requisito de inclusividad de 6.4. Tiene pareja, y eso no la saca del target: lo que la trae a la app es suyo, no de la pareja (ver 5.2).
...
- **La pareja estable es un freno, no un target.** Contradice parcialmente varios de los perfiles trabajados en el sprint, que asumían a la pareja aburrida como cliente natural.
  **Matiz sobre este insight.** El dato sale de dos entrevistados, no de una muestra amplia, así que no basta para excluir a nadie y queda pendiente de investigar. Lo que describe es la pareja como unidad: la pareja estable no busca la app como pareja. Pero el producto se dirige a personas, no a parejas, y tener pareja no impide tener dudas o dificultades con la propia sexualidad. Una persona con pareja sí es target.
...
**2. Modal de invitación.** Propone crear cuenta y, desde ahí, hacer el onboarding. Aparece una vez, al entrar en la home después de la edad, y solo a quien no tiene cuenta. Se puede cerrar. Es el primero de los dos escalones del recordatorio: primero la cuenta, y el onboarding detrás.
...
**8. Cómo nos dirigimos a ti.** Selección múltiple: ella, él, elle y sin género. Es múltiple porque hay personas que usan más de un pronombre. Es una preferencia de lenguaje de la interfaz, no una declaración de identidad, y por eso no necesita consentimiento y puede ir antes de pedirlo. Quien la salta recibe textos sin marca de género, que es lo que ve también quien se salta el onboarding entero.
...
**10. Identidad.** Selección única. La opción "Prefiero describirlo yo" abre un campo abierto para quien no se reconozca en ninguna etiqueta.

**11. Orientación.** Selección múltiple. La opción "Prefiero describirlo yo" abre un campo abierto.

**12. Estado relacional.** Selección única, con opciones sin marca de género ("Sin pareja"). La opción "Prefiero describirlo yo" abre un campo abierto. Cubre estructuras de relación que no son la pareja monógama, que es lo que pide el requisito de inclusividad de 6.4.

**13. Temas.** Selección múltiple con buscador: cómo funciona el cuerpo, comunicación en pareja, deseo y fantasías, límites, consentimiento, masturbación, celos, autoconocimiento, orientación sexual, modelos de relación, dolor, placer, salud sexual, identidad de género, imagen corporal y primeras veces. Es lo que al usuario le apetece leer. La lista es larga a propósito: comunica que aquí se puede hablar de todo.

**14. Qué te ha traído aquí.** Selección múltiple, con la opción de no decirlo y un campo abierto al final para escribirlo con sus palabras. No es una segunda lista de temas: la 13 pregunta qué le interesa y esta pregunta por qué está aquí, que es algo que el usuario trae de fuera y suele ser lo que más le pesa. Va la última porque es la más íntima de las dos partes.
...
**16. Resumen.** Cierra el onboarding de quien ha respondido a las preguntas sensibles. Solo lo ve quien ha entrado en la parte 2, es decir, quien ha encendido el primer permiso, aunque haya respondido solo a algunas preguntas. Quien no lo enciende no tiene nada que resumir y pasa directamente a la home.
...
- El resumen solo lo ve quien ha entrado en la parte 2 (primer permiso encendido).
...
| --- | --- | --- | --- |
| US-01 | Como usuario, quiero ver quién firma o revisa lo que estoy leyendo, para no tener que decidir yo si me fío | Must | Sostiene el diferenciador principal. |
| US-02 | Como usuario, quiero ver de dónde sale un dato en el mismo sitio donde lo leo, para no tener que salir a comprobarlo | Must | Hoy solo existe en el detalle de mitos, que tiene un bloque "dato" con campo de fuente. Artículos, podcasts y vídeos tienen equivalente que se refiere al autor del contenido |
...
- **Usuario que no responde nada en el onboarding.** Ya resuelto: si no enciende el primer permiso, el resumen se salta. Falta definir qué contenido ve en la home alguien de quien no sabemos nada, porque ahora mismo la home de alguien que hace el acceso y alguien que no lo hace es identico, por falta de tiempo se hizo asi, pero para la demo estaria bien para ensenar la personalizacion.
...
**Animaciones.**  Transiciones y animaciones de CSS para lo que se mueve dentro de una pantalla; View Transitions API para los cambios de pantalla; animaciones ligadas al scroll solo con CSS. Este metodo no ha sido analizado, puede que de errores.
...
**La reproducción es simulada.** Al pulsar play aparece el mini player, pero no hay ningún archivo detrás: no suena nada y la barra de progreso no avanza. Es suficiente para la demo mientras nadie espere oír audio. Si en algún momento del proyecto hay margen, un archivo corto por formato convierte la simulación en reproducción real.
...
> 
- Las “soluciones” sean muy generalizadas y no lleguen bien a cada usuario
- los jovenes-adultos estarán dispuestos a  cambiar un hábito ya instalado (google, porno, amigos) por algo nuevo, aunque sepan es que menos fiable?
