...
function pintaBotonFiltros() {
  ...
  var n = temasActivos.length + (formatoActivo() === "todos" ? 0 : 1);
...
function ponFormato(formato) {
  document.querySelectorAll(".chips--formato .chip").forEach(function (c) {
    c.setAttribute("aria-pressed", String(c.dataset.formato === formato));
  });
}
...
function hojaDeFiltros() {
  ...
  /* Formato: uno solo, como fuera. Tocar el elegido lo quita. */
  formatos.addEventListener("click", function (e) {
    var chip = e.target.closest(".chip");
    if (!chip) return;
    var estaba = chip.getAttribute("aria-pressed") === "true";
    formatos.querySelectorAll(".chip").forEach(function (c) { c.setAttribute("aria-pressed", "false"); });
    chip.setAttribute("aria-pressed", String(!estaba));
  });

  /* Temas: varios a la vez. */
  ...
  document.getElementById("filtros-ver").addEventListener("click", function () {
    var elegido = formatos.querySelector('.chip[aria-pressed="true"]');
    ponFormato(elegido ? elegido.dataset.formato : "todos");
    temasActivos = Array.from(temas.querySelectorAll('.chip[aria-pressed="true"]')).map(function (c) { return c.dataset.tema; });
    pintaBiblioteca(formatoActivo());
  });

  /* Tus temas en la home: abren la Biblioteca con ese tema ya elegido. */
  document.addEventListener("click", function (e) {
    var tema = e.target.closest(".tema[data-tema]");
    if (!tema) return;
    ponFormato("todos");
