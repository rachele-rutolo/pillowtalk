...
Todo el contenido visible tiene que ser coherente con esta persona: **Lola**. Su ficha completa (edad, pronombres, identidad, orientación, situación, temas, qué la ha traído y permisos) está en el PRD, sección 5.1, "Persona de la demo". Si cambia, se cambia allí.
...
35 pantallas. El nombre de cada una es el del frame en Figma, salvo **Términos de uso** y **Política de privacidad**, que solo existen en el HTML.
...
**Edad y términos**
Fecha de nacimiento en tres campos (DD / MM / AAAA) y, encima del botón Entrar, una casilla para aceptar los términos de uso y la política de privacidad. Los dos nombres son enlaces a sus pantallas.
...
- Entrar → Home · genérica, y medio segundo después sube **Modal · crear cuenta** (una sola vez, y solo a quien no tiene cuenta).
- términos de uso → **Términos de uso**. política de privacidad → **Política de privacidad**. Atrás vuelve con la fecha y la casilla como estaban.
...
- En la genérica, la franja superior tiene un disparador invisible (`disparador demo · abre el modal de cuenta`) que abre **Modal · crear cuenta**. Es un recurso de demo: permite al presentador volver a abrir el modal cuando quiera sin que se vea el clic.
...
**Onboarding · pronombres** — titular "¿Cómo te hablamos?". Checkbox, selección múltiple: Ella / Él / Elle / Sin género. El subtítulo dice que si la salta, se le habla sin marcar género. Hay un campo "Propón otro pronombre" construido pero oculto en el código (comentado), sin usar por ahora. Es una preferencia de lenguaje de la interfaz, no una declaración de identidad: por eso no necesita consentimiento. Quien la salta recibe textos sin marca de género, y **eso obliga a que toda la copy de la app funcione sin saber el género**.

**Onboarding · permisos** y **Onboarding · permisos · aceptados**
Dos interruptores separados, apagados de partida, y un botón de continuar. Van antes de las preguntas sensibles porque el permiso se pide antes de recoger. Cada título dice la acción, sin ambigüedad: se guardan datos y se usan. Cada texto dice qué datos, para qué y qué pasa si se deja apagado.
- Interruptor 1, **Guardar y usar tus respuestas**: lo que la usuaria responda a continuación.
- Interruptor 2, **Guardar y usar tu actividad en la app**: búsquedas, guardados y respuestas a la pregunta del día.
...
En orden: **identidad** (selección única), **orientación** (chips, selección múltiple), **estado relacional** (selección única; "Sin pareja" en lugar de una opción con género). En las tres, el campo abierto "Escríbelo con tus palabras" solo aparece al elegir "Prefiero describirlo yo", y se vacía si se quita. Después, **temas** (buscador con el componente Text Input y lupa, que filtra los chips mientras se escribe; chips: cómo funciona el cuerpo, comunicación en pareja, deseo y fantasías, límites, consentimiento, masturbación, celos, autoconocimiento, orientación sexual, modelos de relación, dolor, placer, salud sexual, identidad de género, imagen corporal, primeras veces), **qué te ha traído aquí** (checkbox, selección múltiple, con "Prefiero no decirlo" y, al final, campo abierto "Escríbelo con tus palabras").
...
- Sí, avisadme / Ahora no → **Onboarding · resumen** si el primer permiso está encendido; si no, → **Home · genérica**.

**Onboarding · resumen**
Solo lo ve quien ha entrado en la parte 2, es decir, quien ha encendido el primer permiso, aunque haya respondido solo a algunas preguntas. Quien no lo enciende no tiene nada que resumir y pasa directamente a la home.
- Entrar → Home · personalizada.

### App

Están cableadas entre ellas por la navbar y los botones de atrás.
...
**Mini player**
Barra fija sobre la navbar. Empieza oculta. Aparece al dar play a un pódcast o un vídeo, o al abrir un artículo, con el título, la imagen y la duración de esa pieza, que salen del catálogo de contenidos (`PT.CATALOGO` en `js/contenido.js`): una pieza nueva añadida al catálogo funciona sin tocar el mini player. En artículo no lleva botón de reproducir; toda la barra lleva a seguir leyendo. La ✕ la cierra.

**Términos de uso** y **Política de privacidad**
Texto legal de las dos pantallas enlazadas desde Edad y términos. La política de privacidad es donde se explica para qué se pide la fecha de nacimiento.
- Atrás → Edad y términos.
...
5. El resumen solo lo ve quien ha entrado en la parte 2 (primer permiso encendido).
...
- **Medios reproducibles.** La reproducción es simulada: al pulsar play aparece el mini player, pero no hay archivo detrás, no suena nada y la barra de progreso no avanza. Un audio corto y un vídeo corto la convierten en real.
